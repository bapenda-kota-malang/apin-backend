...
import (
	...
	s "github.com/bapenda-kota-malang/apin-backend/internal/services/geojson"
)
...
func GetListGeoJson(w http.ResponseWriter, r *http.Request) {
	var input m.FilterDto
	if hh.ValidateStructByURL(w, *r.URL, &input) == false {
		return
	}

	result, err := s.GetListGeoJson(input)
	hh.DataResponse(w, result, err)
}
...
func GetDetailGeoJson(w http.ResponseWriter, r *http.Request) {
	id := hh.ValidateAutoInc(w, r, "id")
	if id < 1 {
		return
	}

	result, err := s.GetDetailGeoJson(id)
	hh.DataResponse(w, result, err)
}
