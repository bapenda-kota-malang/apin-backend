...
func Create(input m.CreateDto, tx *gorm.DB) (any, error) {
	...
	if err != nil {
		return sh.SetError("request", "create-data", source, "failed", "gagal mengambil menyimpan data datanir: "+err.Error(), data)
	}
...
func CreateBulk(input m.CreateBulkDto) (any, error) {
	...
	err := a.DB.Transaction(func(tx *gorm.DB) error {
		for _, v := range input.Datas {
			...
			data = append(data, respData.(rp.OKSimple).Data.(m.DataNir))
		}
		return nil
	})
