...
func GetList(input m.FilterDto) (any, error) {
	...
	result := a.DB.
		...
		Find(&data)
	if result.Error != nil {
		return sh.SetError("request", "get-data-list", source, "failed", "gagal mengambil data jpb7", data)
	}
...
func GetDetail(id int) (any, error) {
	...
	if result.RowsAffected == 0 {
		...
		return sh.SetError("request", "get-data-detail", source, "failed", "gagal mengambil data jpb7", data)
	}
...
func Update(id int, input m.UpdateDto, tx *gorm.DB) (any, error) {
	...
	if result := tx.Save(&data); result.Error != nil {
		return sh.SetError("request", "update-data", source, "failed", "gagal mengambil menyimpan data jpb7", data)
	}
