...
func GetList(input m.FilterDto) (any, error) {
	...
	result := a.DB.
		Model(&m.PaymentPoint{}).
		Scopes(gh.Filter(input)).
		Count(&count).
		Scopes(gh.Paginate(input, &pagination)).
		Find(&data)
	if result.Error != nil {
		return sh.SetError("request", "get-data-list", source, "failed", "gagal mengambil data payment point", data)
	}
...
func GetDetail(id int) (any, error) {
	...
	if result.RowsAffected == 0 {
		...
		return sh.SetError("request", "get-data-detail", source, "failed", "gagal mengambil data payment point", data)
	}
