...
const source = "kunjungankembali"

func Create(input m.CreateDto, tx *gorm.DB) (any, error) {
	if tx == nil {
		tx = a.DB
	}
	var data m.KunjunganKembali
...
func GetList(input m.FilterDto) (any, error) {
	...
	result := a.DB.
		Model(&m.KunjunganKembali{}).
		Scopes(gh.Filter(input)).
		Count(&count).
		Scopes(gh.Paginate(input, &pagination)).
		Find(&data)
	if result.Error != nil {
		return sh.SetError("request", "get-data-list", source, "failed", "gagal mengambil data kunjungankembali", data)
	}
...
func GetDetail(id int) (any, error) {
	...
	if result.RowsAffected == 0 {
		...
		return sh.SetError("request", "get-data-detail", source, "failed", "gagal mengambil data kunjungankembali", data)
	}
...
func Update(id int, input m.UpdateDto, tx *gorm.DB) (any, error) {
	...
	if result := tx.Save(&data); result.Error != nil {
		return sh.SetError("request", "update-data", source, "failed", "gagal mengambil menyimpan data kunjungankembali", data)
	}
