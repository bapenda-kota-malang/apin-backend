DROP FUNCTION IF EXISTS "public"."penilaian_jpb9"("provinsi_kode" bpchar, "daerah_kode" bpchar, "kecamatan_kode" bpchar, "kelurahan_kode" bpchar, "blok_kode" bpchar, "nourut" bpchar, "jenisop" bpchar, "nobangunan" int8, "tahun" varchar);
CREATE OR REPLACE FUNCTION "public"."penilaian_jpb9"("provinsi_kode" bpchar, "daerah_kode" bpchar, "kecamatan_kode" bpchar, "kelurahan_kode" bpchar, "blok_kode" bpchar, "nourut" bpchar, "jenisop" bpchar, "nobangunan" int8, "tahun" varchar)
  RETURNS "pg_catalog"."numeric" AS $BODY$
declare
  nilai_jpb numeric := 0;
  nilai_jpb9 numeric := 0;
  kls_jpb9 char(1);
  nilai_dbkb_jpb9	numeric := 0;
  kd_jpb char(2);
  thn_dibangun char(4);
  thn_renovasi char(4);
  luas_bng numeric := 0;
...
begin
  ...
  if found then
    ...
    BEGIN
      SELECT "NilaiDbkbJpb9"
      INTO nilai_dbkb_jpb9
      FROM "DbkbJpb9"
      WHERE "Provinsi_Kode" = Provinsi_Kode AND
      "Daerah_Kode" = Daerah_Kode AND
      "TahunDbkbJpb9" = Tahun AND
      "KelasDbkbJpb9" = kls_jpb9 AND
      "LantaiMinJpb9" <= jml_lantai_bng AND
      "LantaiMaxJpb9" >= jml_lantai_bng;
    ...
    nilai_jpb9 := nilai_stl_susut + nilai_fas3;
  
  ELSE
    nilai_jpb9 := 0;
  end if;

  nilai_jpb = nilai_jpb9;
